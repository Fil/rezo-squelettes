...
function sphinx_search($query, $conf = array()) {

spip_timer('search');

include_spip ("inc/charsets");
if (!is_utf8($query)) $query = unicode_to_utf_8(charset2unicode($query));
...
$res = $cl->Query ( $q, $index );
	...
	foreach ($res["matches"] as &$m) {
		$m['weight'] = $m['weight'] / (100 + sqrt(time()+3600*24*20 - intval($m['attrs']['date'])));
	}
