...
function titre_rub($id_rubrique) {
	...
	if (!isset($brut)) {
		...
		foreach (sql_allfetsel(['titre', 'id_rubrique', 'lang'], ['spip_rubriques']) as $t) {
			$brut[$t['id_rubrique']] = [$t['titre'], $t['lang']];
		}
	}
...
function mots_article($id_article, $wrap = '%s', $sep = ', ') {
	...
	if (!isset($liens)) {
		include_spip('base/abstract_sql');
		foreach (sql_allfetsel(['id_mot', 'id_article'], ['spip_mots_articles']) as $t) {
			$liens[intval($t['id_article'])][] = intval($t['id_mot']);
		}
		foreach (sql_allfetsel(['id_mot', 'titre'], ['spip_mots']) as $t) {
	...
	foreach ($liens[$id_article] as $id_mot) {
		$mots[] = sprintf($wrap, $titres[$id_mot]);
	}  # on ne fait pas typo()
...
function microcache($id, $fond, $calcul = false) {
	$cle = "$fond-$id";
	$ttl = 60 * 60;
	if ($calcul || in_array($_GET['var_mode'], ['recalcul', 'debug']) || !($contenu = cache_get($cle))) {
