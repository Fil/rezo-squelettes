...
function optimiser_base_disparus($attente = 86400) {
	...
	$res = sql_select(
		'auteurs_rubriques.id_rubrique AS id',
	...
	$res = sql_select(
		'mots_rubriques.id_rubrique AS id',
	...
	$res = sql_select(
		'auteurs_articles.id_article AS id',
	...
	$res = sql_select(
		'mots_articles.id_article AS id',
	...
	# ######## pour rezo avec la view : PAS TOUCHE
	/*
	...
	$res = sql_select(
		'auteurs_articles.id_auteur AS id',
		'spip_auteurs_articles AS auteurs_articles
	...
	$res = sql_select(
		'auteurs_messages.id_auteur AS id',
	...
	while ($t = sql_fetch($r)) {
		...
		while ($row = sql_fetch($sel)) {
			sql_delete('spip_documents_liens', ['id_document=' . $row['id'], 'id_objet=' . $row['id_objet'], 'objet=' . sql_quote($type)]);
